...
class UnitOfWork:
    """
    Архитектурный паттерн UNIT OF WORK.  Реализуем транзакцию
    """
    # Работает с конкретным потоком
    current = local()

    def __init__(self):
        # инициируем списки для операций
        self.new_objects = []  # создание
        self.dirty_objects = []  # изменение
        self.removed_objects = []  # удаление
